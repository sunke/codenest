package net.codenest.javaandkotlin;
...
//    }
    ...
    public void createStream() {
        String[] colors = {"black", "white", "blue", "red", "yellow", "green"};

        // create stream from array
        assertEquals(colors.length, Stream.of(colors).count());

        // create stream from list
        assertEquals(colors.length, Arrays.asList(colors).stream().count());

        // create stream from elements using Stream.Builder
        Stream.Builder<String> builder = Stream.builder();
        Arrays.asList(colors).forEach(builder::accept);
        Stream<String> stream = builder.build();
        assertEquals(colors.length, stream.count());
    }
